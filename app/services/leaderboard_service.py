...
class LeaderboardService:
    ...
    async def _get_cached(self, key: str) -> Optional[dict]:
        if not self.redis:
            return None
        cached = await self.redis.get_value(key)
        return json.loads(cached) if cached and isinstance(cached, str) else cached

    async def _set_cache(self, key: str, data: str) -> None:
        if self.redis:
            await self.redis.set_value(key, data, expire=self.CACHE_TTL)

    async def get_best_by_rating(self) -> Optional[BestPerformerResponse]:
        cache_key = "lb:best:rating"
    ...
    async def get_top_by_rating(self, limit: int = 10) -> BestPerformersListResponse:
        cache_key = f"lb:top:rating:{limit}"
        cached = await self._get_cached(cache_key)
        if cached:
    ...
    async def get_top_by_delta(self, limit: int = 10) -> BestPerformersListResponse:
        cache_key = f"lb:top:delta:{limit}"
        cached = await self._get_cached(cache_key)
        if cached:
            return BestPerformersListResponse(**cached)
    ...
    async def get_top_by_questions_solved(self, limit: int = 10) -> BestPerformersListResponse:
        cache_key = f"lb:top:questions:{limit}"
        cached = await self._get_cached(cache_key)
        if cached:
            return BestPerformersListResponse(**cached)
    ...
    async def get_top_by_streak(self, limit: int = 10) -> BestPerformersListResponse:
        cache_key = f"lb:top:streak:{limit}"
        cached = await self._get_cached(cache_key)
        if cached:
            return BestPerformersListResponse(**cached)
    ...
    async def get_best_by_streak(self) -> Optional[BestPerformerResponse]:
        cache_key = "lb:best:streak"
        cached = await self._get_cached(cache_key)
        if cached:
            return BestPerformerResponse(**cached)
