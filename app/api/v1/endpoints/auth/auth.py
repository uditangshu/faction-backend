...
from app.api.v1.dependencies import AuthServiceDep, CurrentUserDep, DBSession
...
async def register_push_token(
    ...
    """
    await auth_service.register_push_token(db, str(current_user.id), request.push_token)
    return {"success": True}
...
async def logout(
    ...
    await auth_service.logout(db, str(current_user.id), session_id)
